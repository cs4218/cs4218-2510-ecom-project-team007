...
OUTPUT_FILE = 'results/admin/volume-test-results.png'
...
endpoints = ['get-product', 'create-product', 'update-product', 'delete-product', 'all-orders', 'order-status']
endpoint_labels = ['Get All\nProducts', 'Create\nProduct', 'Update\nProduct', 'Delete\nProduct', 'Get All\nOrders', 'Update\nOrder Status']

data = {volume: [] for volume in volumes}

# Extract p95 data for each volume across all endpoints
for volume in volumes:
    with open(f'results/admin/summary-{volume}.json', 'r') as f:
        summary = json.load(f)
...
ax.set_title('Admin Endpoint Performance vs Product Volume (25 Concurrent Users)',
